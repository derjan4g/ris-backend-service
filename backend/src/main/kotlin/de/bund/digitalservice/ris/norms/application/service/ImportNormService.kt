...
@Service
class ImportNormService(private val parseJurisXmlAdapter: ParseJurisXmlOutputPort, private val saveNormAdapter: SaveNormOutputPort) : ImportNormUseCase {
    override fun importNorm(command: ImportNormUseCase.Command): Mono<UUID> {
        ...
        return parseJurisXmlAdapter.parseJurisXml(parseCommand).map { guid }
//        val norm = createNorm(guid, command.zipFile)
//        val saveCommand = SaveNormOutputPort.Command(norm)
//        return saveNormPort.saveNorm(saveCommand).map { guid }
    }
}

// private fun createNorm(guid: UUID, data: ImportNormUseCase.Command.NormData): Norm {
